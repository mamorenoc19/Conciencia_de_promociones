...
import styles from './resultado-tarjeta-verde.module.css'
import { Link } from "react-router-dom";
//import {useState} from 'react'
//import { render } from '@testing-library/react';
function ResultadosTarjetaVerde(){

    const datos = JSON.parse(window.localStorage.getItem("datosVerde") || '{}');
    let { value, numQuotas } = datos

    let interestPayment=0,capPayment=0;

    const cuota = value * (Math.pow(1+2.8/100,numQuotas)*2.8/100)/(Math.pow(1+2.8/100,numQuotas)-1);
    interestPayment = parseFloat(value*(2.8/100));
...
export default ResultadosTarjetaVerde;