...
import styles from './resultado-tarjeta-azul.module.css'
import { Link } from "react-router-dom";
//import {useState} from 'react'
//import { render } from '@testing-library/react';
function ResultadosTarjetaAzul(){

    const datos = JSON.parse(window.localStorage.getItem("datosAzul") || '{}');
    let { value, numQuotas } = datos

    let interestPayment=0,capPayment=0;

    const cuota = value * (Math.pow(1+3.3/100,numQuotas)*3.3/100)/(Math.pow(1+3.3/100,numQuotas)-1);
    interestPayment = parseFloat(value*(3.3/100));
...
export default ResultadosTarjetaAzul;